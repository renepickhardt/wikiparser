...
public class LogItem {
    ...
    boolean isTitleAnIpAddress(){
        try {
            if (title.substring(9).matches(IPV4_REGEX))return true;
            if (title.substring(9).matches(IPV4_NETWORK_REGEX))return true;
            if (title.substring(9).matches(IPV6_REGEX))return true;
        } catch(Exception e){
            return false;
        }
        return false;
